...
class Base(DeclarativeBase):
    ...
    async def get_or_none(
        cls: Type[T], object_id: int, session: AsyncSession
    ) -> T | None:
    ...
    @classmethod
    async def get(cls: Type[T], object_id: int, session: AsyncSession) -> T:
        result: T | None = await cls.get_or_none(object_id, session)
        if not result:
            raise HTTPException(
